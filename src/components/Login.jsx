...
const Login = ({setToken}) => {
  ...
  const onSubmitHandler = async (e) => {
    try {
      e.preventDefault()
      setToken('okeey') // dummy token
      
      //after the backend is done set this and remove the dummy token
      ...
      <div className='bg-white shadow-md rounded-lg px-8 py-6 max-w-md'>
        ...
        <form>
          ...
          <div className='mb-3 min-w-72'>
            ...
            <button onClick={onSubmitHandler} className='mt-2 w-full py-2 px-4 rounded text-white bg-black' type='submit'>Login</button>
        </form>
      </div>
    </div>
  )
